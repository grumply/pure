...
{-# INLINE rep #-}
rep :: forall (p :: *). (Typeable p) => Proxy p -> JSText
rep _ =
  toText $ go False (typeOf (undefined :: p))
  where
    go surround tr =
...
{-# INLINE fullRep #-}
fullRep :: forall (p :: *). (Typeable p) => Proxy p -> JSText
fullRep _ =
  toText $ go False (typeOf (undefined :: p))
  where
    go surround tr =
...
{-# INLINE qualRep #-}
qualRep :: forall (p :: *). (Typeable p) => Proxy p -> JSText
qualRep _ =
  toText $ go False (typeOf (undefined :: p))
  where
    go surround tr =
