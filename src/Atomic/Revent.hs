...
data Revent k
  = Revent
      ...
      , reventBufferSetter :: Signaled -> k
      }
  | GetReventBuffer (Signaled -> k)
  | SetReventBuffer Signaled k

instance Functor Revent where
  fmap f (Revent sk s) = Revent (fmap f sk) (fmap f s)
  fmap f (GetReventBuffer sk) = GetReventBuffer (fmap f sk)
  fmap f (SetReventBuffer b k) = SetReventBuffer b (f k)
...
setReventBuffer :: (MonadIO c, '[Revent] <: ms)
                => Signaled -> Code ms c ()
setReventBuffer s = Send (SetReventBuffer s (Return ()))

instance Delta Revent Revent where
  delta eval Revent{..} (GetReventBuffer sk) = delta eval reventBuffer sk
  delta eval Revent{..} (SetReventBuffer s k) = delta eval reventBufferSetter (s,k)

-- | Create a Revent Trait from a given `Signaled` buffer.
revent :: (MonadIO c, '[Revent] <. ts)
       => Signaled -> Revent (Action ts c)
revent buf = Revent (buf,return) $ \s o -> pure (Module (revent s) o)
